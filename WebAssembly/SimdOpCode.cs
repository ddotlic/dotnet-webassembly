...
public enum SimdOpCode : byte
{
    ...
    /// <summary>
    ///     SIMD signed pairwise add of 8 16-bit ints producing 4 32-bit ints.
    /// </summary>
    [OpCodeCharacteristics("i32x4.extadd_pairwise_i16x8_s")]
    [SimdInstructionGenerate<Vec128ExtAddPairwise>]
    [SimdOpTraits(hasMethodInfo: false)]
    Int32X4ExtAddPairwiseI16X8Signed = 0x7e,

    /// <summary>
    ///     SIMD unsigned pairwise add of 8 16-bit ints producing 4 32-bit ints.
    /// </summary>
    [OpCodeCharacteristics("i32x4.extadd_pairwise_i16x8_u")]
    [SimdInstructionGenerate<Vec128ExtAddPairwise>]
    [SimdOpTraits(hasMethodInfo: false)]
    Int32X4ExtAddPairwiseI16X8Unsigned = 0x7f,
}
...
internal static class SimdOpCodeExtensions
{
    ...
    internal static MethodInfo FindVector128Getter(string name, Type laneType) =>
        typeof(Vector128<>).MakeGenericType(laneType)
        ...
        return !opName.EndsWith("_s", StringComparison.InvariantCulture)
            ...
            : methodName switch
            {
                "LessThan"
                    ...
                    or "ShiftRightArithmetic" => laneType switch
                    {
                        "i8x16" => typeof(sbyte),
                        "i16x8" => typeof(short),
                        "i32x4" => typeof(int),
    ...
    private sealed record KnownMethods(
        MethodInfo Zero,
        MethodInfo VecEquals,
        MethodInfo OnesComplement,
        MethodInfo ExtractMostSignificantBits);
}
